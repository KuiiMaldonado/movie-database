...
protocol AiringTodayViewControllerProtocol {
    func getAiringTodayPrograms()
}

class AiringTodayViewController: UIViewController {
    var delegate: AiringTodayViewControllerProtocol!
    //MARK: - Properties
    var models : [TVEntity] = [] {
        didSet {
            loadDataInUI()
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        commonInit()
        delegate.getAiringTodayPrograms()
    }
...
extension AiringTodayViewController : UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        print("CAFV AIRING TODAY -> El elemento en la posición \(indexPath.row) fue seleccionado")
    }
}
